...
\u2502`).replace(/││/g,`\u2502
\u2502`).replace(/│╰/g,`\u2502
\u2570`).replace(/⚠️ {2}failed to check/g,`${import_os.EOL}\u26A0\uFE0F  failed to check`);var logger20=console,LOG_FILE_NAME="migration-storybook.log",LOG_FILE_PATH=(0,import_path12.join)(process.cwd(),LOG_FILE_NAME),TEMP_LOG_FILE_PATH="",originalStdOutWrite=process.stdout.write.bind(process.stdout),originalStdErrWrite=process.stderr.write.bind(process.stdout),augmentLogsToFile=()=>{TEMP_LOG_FILE_PATH=import_tempy.default.file({name:LOG_FILE_NAME});let logStream=(0,import_fs_extra11.createWriteStream)(TEMP_LOG_FILE_PATH);process.stdout.write=d=>(originalStdOutWrite(d),logStream.write(cleanLog(d))),process.stderr.write=d=>logStream.write(cleanLog(d))},cleanup=()=>{process.stdout.write=originalStdOutWrite,process.stderr.write=originalStdErrWrite};var logAvailableMigrations=()=>{let availableFixes=allFixes.map(f=>import_chalk24.default.yellow(f.id)).join(", ");logger20.info(`
The following migrations are available: ${availableFixes}`)},automigrate=async({fixId,fixes:inputFixes,dryRun,yes,useNpm,packageManager:pkgMgr,list,configDir:userSpecifiedConfigDir,renderer:rendererPackage,skipInstall}={})=>{var _a;if(list)return logAvailableMigrations(),null;useNpm&&(useNpmWarning(),pkgMgr="npm");let selectedFixes=inputFixes||allFixes,fixes=fixId?selectedFixes.filter(f=>f.id===fixId):selectedFixes;if(fixId&&fixes.length===0)return logger20.info(`\u{1F4ED} No migrations found for ${import_chalk24.default.magenta(fixId)}.`),logAvailableMigrations(),null;augmentLogsToFile();let packageManager=JsPackageManagerFactory.getPackageManager({force:pkgMgr}),{configDir:inferredConfigDir,mainConfig:mainConfigPath,version:storybookVersion}=(0,import_core_common6.getStorybookInfo)(packageManager.retrievePackageJson(),userSpecifiedConfigDir);if(!(storybookVersion&&((_a=import_semver22.default.coerce(storybookVersion))==null?void 0:_a.version)))return logger20.info(import_ts_dedent26.default`
...
For more information visit:`,import_chalk25.default.cyan("https://storybook.js.org")),projectType==="ANGULAR"?(logger21.log(`
To run your Storybook, type:
`),codeLog([`ng run ${installResult.projectName}:storybook`])):projectType==="REACT_NATIVE"?(logger21.log(),logger21.log(import_chalk25.default.yellow(`NOTE: installation is not 100% automated.
...
`),logger21.log(import_chalk25.default.cyan("https://github.com/storybookjs/react-native")),logger21.log()):(logger21.log(`
To run your Storybook, type:
